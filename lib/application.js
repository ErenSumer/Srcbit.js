...
const ViewSet = require("./views/viewsets");
...
class SourceBit {
  constructor(config = {}) {
    this.config = {
      ...
      rateLimit: false,
      cacheTTL: 60000,
      mongoCluster: "mongodb://localhost:27017",
      mongoDbName: "myapp",
      ...config,
    };
    this.requestLogs = {};
  ...
  setupServerStatusRoute() {
    const statusHtml = fs.readFileSync(path.join(__dirname, 'core', 'templates', 'server-status.html'), 'utf8');

    

    this.get('/logs', (req, res) => {
        res.header('Content-Type', 'text/html').send(statusHtml);
    });
  ...
  async connectToDatabase(url, dbName, poolSize = 10) {
    this.database = new Database(url, dbName);
    this.database.setPoolSize(poolSize);
  ...
  ws(event, handler) {
    if (!this.wsManager) {
      throw new Error("WebSocket server not initialized. Call start() first.");
    }
  ...
  start() {
    ...
    this.server.listen(this.config.port, () => {
      console.log(`SourceBit server running on port ${this.config.port}`);
    });
  }
}
