...
class Router {
   ...
   * @param {...function} handlers - The route handlers.
   */
  addOptionalRoute(method, path, ...handlers) {
    const optionalPath = path.replace(/\((.*?)\)/g, "(?:$1)?");
    const escapedPath = optionalPath.replace(/\//g, "\\/");
  ...
  /**
   * Add a route that matches only if none of the other routes match.
   * @param {...function} handlers - The route handlers.
   */
  fallback(...handlers) {
  ...
  use(pathOrMiddleware, handlerOrRouter) {
    if (typeof pathOrMiddleware === "function") {
      ...
      pathOrMiddleware &&
      typeof pathOrMiddleware === "object" &&
      pathOrMiddleware.routes
    ) {
      // It's a distributor object
      this.mergeRouter("", pathOrMiddleware);
    } else {
    ...
    try {
      ...
      for (const middleware of this.globalMiddlewares) {
        if (typeof middleware === "function") {
          ...
          await new Promise((resolve) => {
            middleware.handler(request, response, resolve);
          });
        }
